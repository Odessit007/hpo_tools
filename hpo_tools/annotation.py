...
class Annotation:
    ...
    def _annotate(self):
        ...
        if report and self.on_missing_terms == 'raise':
            raise RuntimeError(f'Missing HPO terms found. Check report.')
        if self.on_missing_terms == 'skip':
            proper_entities = [key for key in self.entity2hpo if key not in report]
        else:
            proper_entities = [key for key, hpo_ids in self.entity2hpo.items() if len(report.get(key, [])) < len(hpo_ids)]
